...
def comparar_resultados(img, nombre_img, maxLevel):
    ...
    for i, sp in enumerate(valores_sp):
        for j, sr in enumerate(valores_sr):
            ...
            axes[i, j].axis("off")
    plt.suptitle(
        f"{nombre_img} - maxLevel={maxLevel}",
        fontsize=16
    )
...
comparar_resultados(img1, "Imagen 1", 0)
comparar_resultados(img1, "Imagen 1", 1)

comparar_resultados(img2, "Imagen 2", 0)
comparar_resultados(img2, "Imagen 2", 1)

comparar_resultados(img3, "Imagen 3", 0)
comparar_resultados(img3, "Imagen 3", 1)

i = 0
for img in imagenes:
    i+=1
    for v_maxLevel in valores_maxLevel:
        comparar_resultados(img, f"Imagen {i}", v_maxLevel)
